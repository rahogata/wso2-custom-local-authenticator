...
public class BasicCustomAuthenticator extends AbstractApplicationAuthenticator
    ...
    /**
     * This method is used to process the authentication response. Inside here we check if this is a authentication
     * request coming from oidc flow and then check if the user is in the 'photoSharingRole'.
     */
    @Override
        ...
        if (isAuthenticated) {
            if ("oidc".equalsIgnoreCase(context.getRequestType())) {
                ...
                try {
                    ...
                    authorization =
                        ((AbstractUserStoreManager) userStoreManager).isUserInRole(username, "photoSharingRole");
